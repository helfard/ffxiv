// 2020/06/26:
// Merged to Ver.5.30926. Thanks a lot!!

const LANGS = [
    // 言語リスト
    { id: 'ja', name: '日本語' },
    { id: 'en', name: 'English' },
    { id: 'zh-TW', name: '繁體中文' },
    { id: 'zh-CH', name: '简体中文' },
    { id: 'ko', name: '한국어' },
    //    { id: 'fr', name: 'français' },
    //    { id: 'de', name: 'Deutsch' },
];

// メモ：
// 中文のコードは繁体＝zh-cmn-Hant・簡体＝zh-cmn-Hansが正しい？
// しかしGoogleのサイトはzh-TW・zh-CHとなっている。

// EnglishはChocobo鯖のPaca Kumaさんに添削してもらいました。
// 中文（繁體）はChocobo鯖のRondo Grantさんに作ってもらいました。
// 中文（简体）はPandaemonium鯖のMarcquelort Ourauxさんにチェックしてもらいました。
// 한국어はAegis鯖のPoly Dentさん（초코보鯖の폴리덴트さん）と匿名のヒカセンさんに作ってもらいました。

// ToDo:
// Clearly I need some help to translate to French and German…

const WORDS = {
    // 翻訳辞書
    'title': {
        'ja': 'OiTPKO',
        'en': 'OiTPKO',
        'zh-TW': 'OiTPKO',
        'zh-CH': 'OiTPKO',
        'ko': 'OiTPKO',
        'fr': 'OiTPKO',
        'de': 'OiTPKO',
    },

    'class': {
        'ja': '級',
        'en': 'Class',
        'zh-TW': '級',
        'zh-CH': '级',
        'ko': '등급',
        'fr': '',
        'de': '',
    },
    'preferences': {
        'ja': 'パーツ',
        'en': 'Preferences',
        'zh-TW': '部件',
        'zh-CH': '部件',
        'ko': '부품',
        'fr': '',
        'de': '',
    },
    'all': {
        'ja': '全て',
        'en': 'All',
        'zh-TW': '全部',
        'zh-CH': '全部',
        'ko': '전부',
        'fr': '',
        'de': '',
    },
    'normal all': {
        'ja': '旧パーツ全て',
        'en': 'Normal',
        'zh-TW': '一般部件全部',
        'zh-CH': '一般部件全部',
        'ko': '구 파트 전부',
        'fr': '',
        'de': '',
    },
    'modified all': {
        'ja': '改級パーツ全て',
        'en': 'Modified',
        'zh-TW': '改級部件全部',
        'zh-CH': '改级部件全部',
        'ko': '신 파트 전부',
        'fr': '',
        'de': '',
    },

    'rank': {
        'ja': 'Rank',
        'en': 'Rank',
        'zh-TW': '等級',
        'zh-CH': '等级',
        'ko': '랭크',
        'fr': '',
        'de': '',
    },
    'filter': {
        'ja': 'フィルタ',
        'en': 'Filter',
        'zh-TW': '篩選',
        'zh-CH': '筛选',
        'ko': '필터',
        'fr': '',
        'de': '',
    },
    'preset': {
        'ja': 'プリセット',
        'en': 'Preset',
        'zh-TW': '預設',
        'zh-CH': '预设',
        'ko': '사전 설정',
        'fr': '',
        'de': '',
    },
    'reset': {
        'ja': 'リセット',
        'en': 'Reset',
        'zh-TW': '重新設定',
        'zh-CH': '重新设定',
        'ko': '초기화',
        'fr': '',
        'de': '',
    },
    'memo': {
        'ja': 'MEMO',
        'en': 'MEMO',
        'zh-TW': 'MEMO',
        'zh-CH': 'MEMO',
        'ko': 'MEMO',
        'fr': '',
        'de': '',
    },
    'default memo': {
        'ja': 'MEMO',
        'en': 'MEMO',
        'zh-TW': 'MEMO',
        'zh-CH': 'MEMO',
        'ko': 'MEMO',
        'fr': '',
        'de': '',
    },
    'save': {
        'ja': 'SAVE',
        'en': 'SAVE',
        'zh-TW': 'SAVE',
        'zh-CH': 'SAVE',
        'ko': 'SAVE',
        'fr': '',
        'de': '',
    },
    'load': {
        'ja': 'LOAD',
        'en': 'LOAD',
        'zh-TW': 'LOAD',
        'zh-CH': 'LOAD',
        'ko': 'LOAD',
        'fr': '',
        'de': '',
    },
    'confirm override': {
        'ja': '上書きしますか？',
        'en': 'Override?',
        'zh-TW': 'Override?',
        'zh-CH': 'Override?',
        'ko': 'Override?',
        'fr': '',
        'de': '',
    },
/*
    'comment': {
        'ja': '説明',
        'en': 'Description',
        'zh-TW': '說明',
        'zh-CH': '说明',
        'ko': '기술',
        'fr': '',
        'de': '',
    },
    'default comment': {
        'ja': 'ここに説明が出ます。',
        'en': 'Description here.',
        'zh-TW': '解說在這裡',
        'zh-CH': '解说在这里',
        'ko': '여기에 설명이 나옵니다.',
        'fr': '',
        'de': '',
    },
*/

    'dock': {
        'ja': '造船ドック',
        'en': 'Dock',
        'zh-TW': 'Dock',
        'zh-CH': 'Dock',
        'ko': 'Dock',
        'fr': '',
        'de': '',
    },
    'built list': {
        'ja': '建造済み',
        'en': 'Assembled',
        'zh-TW': 'Assembled',
        'zh-CH': 'Assembled',
        'ko': 'Assembled',
        'fr': '',
        'de': '',
    },
    'plan list': {
        'ja': '候補一覧',
        'en': 'Plan',
        'zh-TW': 'Plan',
        'zh-CH': 'Plan',
        'ko': 'Plan',
        'fr': '',
        'de': '',
    },
    'empty dock': {
        'ja': 'Empty..',
        'en': 'Empty..',
        'zh-TW': 'Empty..',
        'zh-CH': 'Empty..',
        'ko': 'Empty..',
        'fr': '',
        'de': '',
    },
    'result list': {
        'ja': '構成一覧',
        'en': 'Results',
        'zh-TW': '組成一覽',
        'zh-CH': '组成一览',
        'ko': '구성 목록',
        'fr': '',
        'de': '',
    },
    'no results': {
        'ja': '条件を満たす組み合わせが存在しません。',
        'en': 'No results found.',
        'zh-TW': '沒辦法組出這個條件的組合。',
        'zh-CH': '组成一览',
        'ko': '조건을 만족하는 조합이 존재하지 않습니다.',
        'fr': '',
        'de': '',
    },
    'found': {
        'ja': 'パターンありました。',
        'en': 'submersibles are found.',
        'zh-TW': '找到這個部件了.',
        'zh-CH': '找到这个部件了',
        'ko': '조합이 있었습니다.',
        'fr': '',
        'de': '',
    },
    'too many results': {
        'ja': '組み合わせが多すぎて表示できません。',
        'en': 'There are too many submersibles to show all.',
        'zh-TW': '組合有太多種了無法顯示。',
        'zh-CH': '组合有太多种了无法显示。',
        'ko': '조합이 너무 많아서 표시 할 수 없습니다.',
        'fr': '',
        'de': '',
    },
    'show all results': {
        'ja': '無理矢理表示する！（重い）',
        'en': 'Show all!! (CAUTION: LAGGY!!!)',
        'zh-TW': '我全都要!! (電腦哀號)',
        'zh-CH': '我全都要!! (电脑哀号)',
        'ko': '모두보기!（처리가 느릴 수 있습니다）',
        'fr': '',
        'de': '',
    },
    'limit results': {
        'ja': '縮小表示に戻す。',
        'en': 'Back to limited mode.',
        'zh-TW': '變回精簡模式',
        'zh-CH': '变回精简模式。',
        'ko': '간략 모드로 다시 변경',
        'fr': '',
        'de': '',
    },
    'add to dock': {
        'ja': '一覧をクリックすると造船ドックに入ります。',
        'en': 'Click a submersible on the list to add to the Dock.',
        'zh-TW': 'Click a submersible on the list to add to the Dock.',
        'zh-CH': 'Click a submersible on the list to add to the Dock.',
        'ko': 'Click a submersible on the list to add to the Dock.',
        'fr': '',
        'de': '',
    },
    'remove from dock': {
        'ja': '一覧をクリックすると候補一覧に戻します。',
        'en': 'Click a submersible on the list to remove from the Dock.',
        'zh-TW': 'Click a submersible on the list to remove from the Dock.',
        'zh-CH': 'Click a submersible on the list to remove from the Dock.',
        'ko': 'Click a submersible on the list to remove from the Dock.',
        'fr': '',
        'de': '',
    },
    'copy submersibles': {
        'ja': 'データをコピーする。',
        'en': 'Copy submersibles.',
        'zh-TW': '複製資料。',
        'zh-CH': '复制资料。',
        'ko': '데이터를 복사한다.',
        'fr': '',
        'de': '',
    },
    'copied': {
        'ja': 'コピーしました！',
        'en': 'Copied!',
        'zh-TW': '複製完成!',
        'zh-CH': '复制完成!',
        'ko': '복사했습니다!',
        'fr': '',
        'de': '',
    },

    'hull': {
        'ja': '艦体',
        'en': 'Hull',
        'zh-TW': '船體',
        'zh-CH': '船体',
        'ko': '함체',
        'fr': '',
        'de': '',
    },
    'stern': {
        'ja': '艦尾',
        'en': 'Stern',
        'zh-TW': '船尾',
        'zh-CH': '船尾',
        'ko': '함미',
        'fr': '',
        'de': '',
    },
    'bow': {
        'ja': '艦首',
        'en': 'Bow',
        'zh-TW': '船首',
        'zh-CH': '船首',
        'ko': '함수',
        'fr': '',
        'de': '',
    },
    'bridge': {
        'ja': '艦橋',
        'en': 'Bridge',
        'zh-TW': '艦橋',
        'zh-CH': '舰桥',
        'ko': '함교',
        'fr': '',
        'de': '',
    },

    'cost': {
        'ja': 'パーツコスト',
        'en': 'Components',
        'zh-TW': '配件重量',
        'zh-CH': '配件重量',
        'ko': '구성품',
        'fr': '',
        'de': '',
    },
    'surveillance': {
        'ja': '探査性能',
        'en': 'Surveillance',
        'zh-TW': '探索性能',
        'zh-CH': '探索性能',
        'ko': '탐사 성능',
        'fr': '',
        'de': '',
    },
    'retrieval': {
        'ja': '収集性能',
        'en': 'Retrieval',
        'zh-TW': '收集性能',
        'zh-CH': '收集性能',
        'ko': '수집 성능',
        'fr': '',
        'de': '',
    },
    'speed': {
        'ja': '巡航速度',
        'en': 'Speed',
        'zh-TW': '巡航速度',
        'zh-CH': '巡航速度',
        'ko': '순항 속도',
        'fr': '',
        'de': '',
    },
    'range': {
        'ja': '航続距離',
        'en': 'Range',
        'zh-TW': '航行距離',
        'zh-CH': '航行距离',
        'ko': '항속 거리',
        'fr': '',
        'de': '',
    },
    'favor': {
        'ja': '運',
        'en': 'Favor',
        'zh-TW': '恩惠（運氣）',
        'zh-CH': '恩惠（运气）',
        'ko': '운',
        'fr': '',
        'de': '',
    },

    'cst': {
        'ja': 'Cost',
        'en': 'Cost',
        'zh-TW': 'Cost',
        'zh-CH': 'Cost',
        'ko': '비용',
        'fr': '',
        'de': '',
    },
    'srv': {
        'ja': '探査',
        'en': 'Srv',
        'zh-TW': '探索',
        'zh-CH': '探索',
        'ko': '탐사',
        'fr': '',
        'de': '',
    },
    'rtr': {
        'ja': '収集',
        'en': 'Rtr',
        'zh-TW': '收集',
        'zh-CH': '收集',
        'ko': '수집',
        'fr': '',
        'de': '',
    },
    'spd': {
        'ja': '速度',
        'en': 'Spd',
        'zh-TW': '速度',
        'zh-CH': '速度',
        'ko': '속도',
        'fr': '',
        'de': '',
    },
    'rng': {
        'ja': '距離',
        'en': 'Rng',
        'zh-TW': '距離',
        'zh-CH': '距离',
        'ko': '거리',
        'fr': '',
        'de': '',
    },
    'fvr': {
        'ja': '運',
        'en': 'Fav',
        'zh-TW': '運',
        'zh-CH': '运',
        'ko': '운',
        'fr': '',
        'de': '',
    },

    'shark': {
        'ja': 'シャーク',
        'en': 'Shark',
        'zh-TW': '鯊魚',
        'zh-CH': '鲨鱼',
        'ko': '상어급',
        'fr': '',
        'de': '',
    },
    'unkiu': {
        'ja': 'ウンキウ',
        'en': 'Unkiu',
        'zh-TW': '甲鱉',
        'zh-CH': '甲鳖',
        'ko': '철갑게급',
        'fr': '',
        'de': '',
    },
    'whale': {
        'ja': 'ホエール',
        'en': 'Whale',
        'zh-TW': '鬚鯨',
        'zh-CH': '须鲸',
        'ko': '고래급',
        'fr': '',
        'de': '',
    },
    'coelacanth': {
        'ja': 'シーラカンス',
        'en': 'Coelacanth',
        'zh-TW': '腔棘魚',
        'zh-CH': '腔棘鱼',
        'ko': '실러캔스급',
        'fr': '',
        'de': '',
    },
    'syldra': {
        'ja': 'シルドラ',
        'en': 'Syldra',
        'zh-TW': '希爾德拉',
        'zh-CH': '希尔德拉',
        'ko': '실드라급',
        'fr': '',
        'de': '',
    },
    'modified shark': {
        'ja': 'シャーク改',
        'en': 'M-Shark',
        'zh-TW': '鯊魚改',
        'zh-CH': '鲨鱼改',
        'ko': '신 샤크급',
        'fr': '',
        'de': '',
    },
    'modified unkiu': {
        'ja': 'ウンキウ改',
        'en': 'M-Unkiu',
        'zh-TW': '甲鱉改',
        'zh-CH': '甲鳖改',
        'ko': '신 철갑게급',
        'fr': '',
        'de': '',
    },
    'modified whale': {
        'ja': 'ホエール改',
        'en': 'M-Whale',
        'zh-TW': '鬚鯨改',
        'zh-CH': '须鲸改',
        'ko': '신 고래급',
        'fr': '',
        'de': '',
    },
    'modified coelacanth': {
        'ja': 'シーラカンス改',
        'en': 'M-Coelacanth',
        'zh-TW': '腔棘魚改',
        'zh-CH': '腔棘鱼改',
        'ko': '신 실러캔스급',
        'fr': '',
        'de': '',
    },
    'modified syldra': {
        'ja': 'シルドラ改',
        'en': 'M-Syldra',
        'zh-TW': '希爾德拉改',
        'zh-CH': '希尔德拉改',
        'ko': '신 실드라급',
        'fr': '',
        'de': '',
    },

    'sha': {
        'ja': 'シャ',
        'en': 'S',
        'zh-TW': '鯊',
        'zh-CH': '鲨',
        'ko': '상어',
        'fr': '',
        'de': '',
    },
    'un': {
        'ja': 'ウン',
        'en': 'U',
        'zh-TW': '鱉',
        'zh-CH': '鳖',
        'ko': '철갑',
        'fr': '',
        'de': '',
    },
    'wha': {
        'ja': 'ホエ',
        'en': 'W',
        'zh-TW': '鯨',
        'zh-CH': '鲸',
        'ko': '고래',
        'fr': '',
        'de': '',
    },
    'coe': {
        'ja': 'シー',
        'en': 'C',
        'zh-TW': '棘',
        'zh-CH': '棘',
        'ko': '실러',
        'fr': '',
        'de': '',
    },
    'syl': {
        'ja': 'シル',
        'en': 'S',
        'zh-TW': '希',
        'zh-CH': '希',
        'ko': '실드',
        'fr': '',
        'de': '',
    },
    'm-sha': {
        'ja': 'ｼｬ改',
        'en': 'S+',
        'zh-TW': '鯊改',
        'zh-CH': '鲨改',
        'ko': '신샤크', // You need not mind if these take more than 3 characters, as you see 2 is the best though.
        'fr': '',
        'de': '',
    },
    'm-un': {
        'ja': 'ｳﾝ改',
        'en': 'U+',
        'zh-TW': '鱉改',
        'zh-CH': '鳖改',
        'ko': '신철갑',
        'fr': '',
        'de': '',
    },
    'm-wha': {
        'ja': 'ﾎｴ改',
        'en': 'W+',
        'zh-TW': '鯨改',
        'zh-CH': '鲸改',
        'ko': '신고래',
        'fr': '',
        'de': '',
    },
    'm-coe': {
        'ja': 'ｼｰ改',
        'en': 'C+',
        'zh-TW': '棘改',
        'zh-CH': '棘改',
        'ko': '신실러',
        'fr': '',
        'de': '',
    },
    'm-syl': {
        'ja': 'ｼﾙ改',
        'en': 'S+',
        'zh-TW': '希改',
        'zh-CH': '希改',
        'ko': '신실드',
        'fr': '',
        'de': '',
    },

    'kamacite ore': {
        'ja': 'カマサイト鉱',
        'en': 'Kamacite Ore',
        'zh-TW': '鐵紋礦石',
        'zh-CH': '铁纹矿石',
        'ko': '카마사이트 광석',
        'fr': '',
        'de': '',
    },
    'cocobolo lumber': {
        'ja': 'ココボロ材',
        'en': 'Cocobolo Lumber',
        'zh-TW': '黃檀木材',
        'zh-CH': '黄檀木材',
        'ko': '코코볼로 목재',
        'fr': '',
        'de': '',
    },
    'balsa wood scrap': {
        'ja': 'バルサ廃材',
        'en': 'Balsa Wood Scrap',
        'zh-TW': '輕木廢材',
        'zh-CH': '轻木废材',
        'ko': '발사나무 폐재',
        'fr': '',
        'de': '',
    },
    'pure titanium ore': {
        'ja': 'ピュアチタン鉱',
        'en': 'Pure Titanium Ore',
        'zh-TW': '純鈦礦',
        'zh-CH': '纯钛矿',
        'ko': '순티타늄 광석',
        'fr': '',
        'de': '',
    },
    'cryptomeria log': {
        'ja': 'クリプトメリア原木',
        'en': 'Cryptomeria Log',
        'zh-TW': '柳杉原木',
        'zh-CH': '柳杉原木',
        'ko': '낙우송 원목',
        'fr': '',
        'de': '',
    },

    'i': {
        'ja': 'N',
        'en': 'I',
        'zh-TW': 'N',
        'zh-CH': 'N',
        'ko': 'N',
        'fr': '',
        'de': '',
    },
    'ii': {
        'ja': '真',
        'en': 'II',
        'zh-TW': '真',
        'zh-CH': '真',
        'ko': '진',
        'fr': '',
        'de': '',
    },
    'iii': {
        'ja': '極',
        'en': 'III',
        'zh-TW': '極',
        'zh-CH': '极',
        'ko': '극',
        'fr': '',
        'de': '',
    },
    'iv': {
        'ja': '幻',
        'en': 'IV',
        'zh-TW': '幻',
        'zh-CH': '幻',
        'ko': '환',
        'fr': '',
        'de': '',
    },
    'v': {
        'ja': '零式',
        'en': 'V',
        'zh-TW': '零式',
        'zh-CH': '零式',
        'ko': '영식',
        'fr': '',
        'de': '',
    },
    'vi': {
        'ja': '絶',
        'en': 'VI',
        'zh-TW': '絕',
        'zh-CH': '绝',
        'ko': '절',
        'fr': '',
        'de': '',
    },
    'vii': {
        'ja': 'FF16兼任', // It means “Yoshida’s work is harder than 絶”. :)
        'en': 'VII',
        'zh-TW': '吉田爸爸',
        'zh-CH': '吉田爸爸',
        'ko': 'FF16겸임',
        'fr': '',
        'de': '',
    },

    'deepsea y': {
        'ja': '溺没海 Y',
        'en': 'Deep-Sea Site:\nFlickering Dip', // \n means line break.
        'zh-TW': '溺沒海 Y',
        'zh-CH': '溺没海 Y',
        'ko': '익몰해:\n저녁놀 열곡',
        'fr': '',
        'de': '',
    },
    'deepsea yv': {
        'ja': '溺没海 Y→V',
        'en': 'Deep-Sea Site:\nFlickering Dip\n-> The Rimilala Shelf', // “->” = “->”
        'zh-TW': '溺沒海 Y→V',
        'zh-CH': '溺没海 Y→V',
        'ko': '익몰해:\n저녁놀 열곡\n-> 라미라라 해구 급경사면',
        'fr': '',
        'de': '',
    },
    'ashsea ab': {
        'ja': '灰海 A→B',
        'en': 'Sea of Ash:\nWreckage of the Windwalker\n-> South Isle of Zozonan',
        'zh-TW': '灰海 A→B',
        'zh-CH': '灰海 A→B',
        'ko': '잿빛 바다:\n풍운호 잔해\n-> 조조난 섬 남단',
        'fr': '',
        'de': '',
    },
    'ashsea bad': {
        'ja': '灰海 B→A→D',
        'en': 'Sea of Ash:\nWreckage of the Windwalker\n-> South Isle of Zozonan\n-> Sea of Ash 1',
        'zh-TW': '灰海 B→A→D',
        'zh-CH': '灰海 B→A→D',
        'ko': '잿빛 바다:\n풍운호 잔해\n-> 조조난 섬 남단\n-> 잿빛 바다 01',
        'fr': '',
        'de': '',
    },
    'ashsea badf': {
        'ja': '灰海 B→A→D→F',
        'en': 'Sea of Ash:\nWreckage of the Windwalker\n-> South Isle of Zozonan\n-> Sea of Ash 1\n -> Sea of Ash 2',
        'zh-TW': '灰海 B→A→D→F',
        'zh-CH': '灰海 B→A→D→F',
        'ko': '잿빛 바다:\n풍운호 잔해\n-> 조조난 섬 남단\n-> 잿빛 바다 01\n -> 잿빛 바다 02',
        'fr': '',
        'de': '',
    },
    'deepsea gh': {
        'ja': '溺没海 G→H',
        'en': 'Deep-Sea Site:\n-> The Umbrella Narrow\n-> Offender&#39;s Rot',
        'zh-TW': '溺沒海 G→H',
        'zh-CH': '溺没海 G→H',
        'ko': '익몰해:\n-> 우산 해협\n-> 포로섬',
        'fr': '',
        'de': '',
    },
    'deepsea igh': {
        'ja': '溺没海 I→G→H',
        'en': 'Deep-Sea Site:\n-> Neolith Island\n-> The Umbrella Narrow\n-> Offender&#39;s Rot',
        'zh-TW': '溺沒海 I→G→H',
        'zh-CH': '溺没海 I→G→H',
        'ko': '익몰해:\n-> 석인도\n-> 우산 해협\n-> 포로섬',
        'fr': '',
        'de': '',
    },
    'ashsea aci': {
        'ja': '灰海 A→C→I',
        'en': 'Sea of Ash:\nSouth Isle of Zozonan\n-> North Isle of Zozonan\n-> The Central Charnel Trench',
        'zh-TW': '灰海 A→C→I',
        'zh-CH': '灰海 A→C→I→L',
        'ko': '잿빛 바다:\n조조난 섬 남단\n-> 조조난 섬 북단\n-> 화장로 해구 중턱',
        'fr': '',
        'de': '',
    },
    'ashsea acil': {
        'ja': '灰海 A→C→I→L',
        'en': 'Sea of Ash:\nSouth Isle of Zozonan\n-> North Isle of Zozonan\n-> The Central Charnel Trench\n-> The Lone Glove',
        'zh-TW': '灰海 A→C→I→L',
        'zh-CH': '灰海 A→C→I→L',
        'ko': '잿빛 바다:\n조조난 섬 남단\n-> 조조난 섬 북단\n-> 화장로 해구 중턱\n-> 짝짝이 장갑 섬',
        'fr': '',
        'de': '',
    },
    'ashsea acmil': {
        'ja': '灰海 A→C→M→I→L',
        'en': 'Sea of Ash:\nSouth Isle of Zozonan\n-> North Isle of Zozonan\n-> The Midden Pit\n-> The Central Charnel Trench\n-> The Lone Glove',
        'zh-TW': '灰海 A→C→M→I→L',
        'zh-CH': '灰海 A→C→M→I→L',
        'ko': '잿빛 바다:\n조조난 섬 남단\n-> 조조난 섬 북단\n-> 자갈 해분\n-> 화장로 해구 중턱\n-> 짝짝이 장갑 섬',
        'fr': '',
        'de': '',
    },
    'ashsea bdi': {
        'ja': '灰海 B→D→I',
        'en': 'Sea of Ash:\nWreckage of the Windwalker\n-> Sea of Ash 1\n-> The Central Charnel Trench',
        'zh-TW': '灰海 B→D→I',
        'zh-CH': '灰海 B→D→I',
        'ko': '잿빛 바다:\n풍운호 잔해\n-> 잿빛 바다 01\n-> 화장로 해구 중턱',
        'fr': '',
        'de': '',
    },
    'ashsea bdik': {
        'ja': '灰海 B→D→I→K',
        'en': 'Sea of Ash:\nWreckage of the Windwalker\n-> Sea of Ash 1\n-> The Central Charnel Trench\n-> Sea of Ash 4',
        'zh-TW': '灰海 B→D→I→K',
        'zh-CH': '灰海 B→D→I→K',
        'ko': '잿빛 바다:\n풍운호 잔해\n-> 잿빛 바다 01\n-> 화장로 해구 중턱\n-> 잿빛 바다 04',
        'fr': '',
        'de': '',
    },

    /*
    '': {
        'ja': '',
        'en': '',
        'zh-TW': '',
        'zh-CH': '',
        'ko': '',
        'fr': '',
        'de': '',
    },
    */
};


